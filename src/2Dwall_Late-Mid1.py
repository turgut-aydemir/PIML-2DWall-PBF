...
def generate_points(p=[], f=[]):
    ...
    init_data = np.load('Processed_Temperature_Data_20240910_112506_t0.npy').T  # this data is under \data\3_hybrid folder
...
def load_data(p=[], f=[]):
    #
    data = np.load('Processed_Temperature_Data_20240910_112506.npy').T

    ind = (data[:, 2] < 0.8) * (data[:, 1] > 6.5)
    data1 = data[ind, :]
...
##validation data
data = np.load('Processed_Temperature_Data_20240910_112506.npy').T
ind = (data[:, 2] > 0) * (data[:, 2] < 0.8) * (data[:, 1] < 6.5)
data = data[ind, :]

...
def generate_points(p=[], f=[]):
    ...
    init_dataTimeAdjust = np.load('Mid2.npy').T
...
t1_data = init_dataTimeAdjust[indTimeAdjust1, :]
t2_data = init_dataTimeAdjust[indTimeAdjust2, :]
t3_data = init_dataTimeAdjust[indTimeAdjust3, :]
grid_size = 55

# For t=1, t=2, t=3, we are extracting the temperature column and reshaping it
t1_temperature = t1_data[:, 3].reshape(grid_size, grid_size)  # Temperature at t=1
t2_temperature = t2_data[:, 3].reshape(grid_size, grid_size)  # Temperature at t=2
t3_temperature = t3_data[:, 3].reshape(grid_size, grid_size)  # Temperature at t=3
